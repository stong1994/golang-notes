...
func (p *Processor) SetHandler(key string, process IProcess) {
...
func (p *RootProcessor) Process(request *Request) {
	p.Handlers[request.Method].Process(request)
}

func NewRootProcessor() *RootProcessor {
	r := &RootProcessor{}
	r.Init()
	return r
}
...
func NewPostProcessor(mux *Mux) *PostProcessor {
...
func (pp *PostProcessor) Process(request *Request) {
...
func NewMux() *Mux {
	...
	root := NewRootProcessor()
	post := NewPostProcessor(&mux)
